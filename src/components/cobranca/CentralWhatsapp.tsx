...
import { criarCobranca, jaExisteCobrancaHoje, registrarLog, obterConfiguracaoCobranca } from "@/services/cobrancas";
...
export default function CentralWhatsapp() {
  ...
  async function enviar(p:any) { const tipo=modelo(p); if(await jaExisteCobrancaHoje(p.clienteId,tipo)) return alert("Já existe uma cobrança deste tipo para este cliente hoje."); const config=await obterConfiguracaoCobranca(); const provider=config?.providerAtivo||"WAME"; const valores:Record<string,string>={cliente:p.clienteNome||"",produto:p.produtoNome||"",valor:Number(p.valor||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"}),parcela:String(p.parcela||""),totalParcelas:String(p.totalParcelas||""),vencimento:data(p).toLocaleDateString("pt-BR"),empresa:config?.empresa||"Pantoja Phone Imports"}; const mensagem=modelos[tipo].replace(/{{(.*?)}}/g,(_,chave)=>valores[chave]||""); const telefone=p.clienteTelefone||p.telefone||""; const criada=await criarCobranca({clienteId:p.clienteId,cliente:p.clienteNome,telefone,produto:p.produtoNome,parcela:Number(p.parcela),valor:Number(p.valor),vencimento:data(p).toISOString(),tipoMensagem:tipo,status:"PENDENTE",criadoEm:new Date().toISOString(),provider,tentativas:0,responsavel:usuario?.nome,mensagem}); try { const resultado=await whatsappProvider.send({telefone,mensagem,provider}); await registrarLog({cobrancaId:criada.id,data:new Date().toISOString(),usuario:usuario?.nome||"",tipo,resultado:resultado.status}); } catch(error) { await registrarLog({cobrancaId:criada.id,data:new Date().toISOString(),usuario:usuario?.nome||"",tipo,resultado:"ERRO",erro:String(error)}); }
  }
