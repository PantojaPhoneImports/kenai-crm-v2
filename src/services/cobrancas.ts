...
export async function jaExisteCobrancaHoje(clienteId: string | undefined, tipoMensagem: string) { const hoje=new Date().toISOString().slice(0,10); const s=await getDocs(query(collection(db,"cobrancas"),where("clienteId","==",clienteId||""),where("tipoMensagem","==",tipoMensagem))); return s.docs.some(d=>d.data().status === "ENVIADA" && String(d.data().criadoEm||"").startsWith(hoje)); }
...
export async function salvarConfiguracaoCobranca(configuracao: ConfiguracaoCobranca) { const atual=await getDocs(collection(db,"configuracoesCobranca")); if(atual.empty) return addDoc(collection(db,"configuracoesCobranca"),configuracao); return setDoc(doc(db,"configuracoesCobranca",atual.docs[0].id),configuracao,{merge:true}); }
