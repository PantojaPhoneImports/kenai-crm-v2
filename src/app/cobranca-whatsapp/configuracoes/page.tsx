...
import MetaCloudStatus from "@/components/cobranca/MetaCloudStatus";
...
const providers: Array<{ id: ProviderWhatsapp; nome: string; descricao: string }> = [
  ...
  { id: "META_CLOUD_API", nome: "Meta Cloud API", descricao: "Provider oficial server-side com diagnóstico seguro." },
...
export default function ConfiguracoesCobrancaPage() {
  ...
  async function ativar(id: ProviderWhatsapp) {
    if (id !== "EVOLUTION" && id !== "WAME") { setAviso("Use a ativação segura no diagnóstico da Meta."); return; }
  ...
  return <Layout><div className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{providers.map(item => { const ativo = config.providerAtivo === item.id; const bloqueado = !["EVOLUTION", "WAME", "META_CLOUD_API"].includes(item.id); return <article key={item.id} className={`rounded-2xl border bg-zinc-900 p-5 ${ativo ? "border-emerald-400/70" : "border-zinc-800"}`}><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold text-white">{item.nome}</h2><p className="mt-1 text-xs text-zinc-400">{item.descricao}</p></div><span className={`rounded-full px-2 py-1 text-xs ${ativo ? "bg-emerald-500/15 text-emerald-300" : bloqueado ? "bg-amber-500/15 text-amber-300" : "bg-zinc-800 text-zinc-400"}`}>{ativo ? "ATIVO" : bloqueado ? "DESATIVADO" : "DISPONÍVEL"}</span></div>{item.id === "EVOLUTION" && <div className="mt-4"><EvolutionConnection /></div>}{item.id === "META_CLOUD_API" && <MetaCloudStatus onActivated={() => { setConfig(atual => ({ ...atual, providerAtivo: "META_CLOUD_API" })); setAviso("Meta Cloud API está ativa."); }} />}{item.id !== "META_CLOUD_API" && <Button className="mt-4" size="sm" onClick={() => ativar(item.id)} disabled={bloqueado || salvando === item.id}>{salvando === item.id ? <Loader2 className="animate-spin" size={14} /> : <CheckCircle2 size={14} />} Ativar</Button>}</article>; })}</section>
