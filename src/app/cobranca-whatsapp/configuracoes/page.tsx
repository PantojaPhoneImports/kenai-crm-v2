...
const providers: Array<{ id: ProviderWhatsapp; nome: string; descricao: string }> = [
  { id: "WAME", nome: "WA.ME", descricao: "Abertura manual segura pelo WhatsApp Web." },
  { id: "EVOLUTION", nome: "Evolution API", descricao: "Preparado para integração por API." },
  { id: "META_CLOUD", nome: "Meta Cloud", descricao: "Preparado para a API oficial da Meta." },
  { id: "Z_API", nome: "Z-API", descricao: "Preparado para integração por API." },
  { id: "ULTRAMSG", nome: "UltraMsg", descricao: "Preparado para integração por API." },
];
...
export default function ConfiguracoesCobrancaPage() {
  ...
  useEffect(() => { obterConfiguracaoCobranca().then(valor => { if (valor) setConfig({ ...padrao, ...valor, providers: valor.providers || {} }); }).finally(() => setCarregando(false)); }, []);
  const detalhe = (id: ProviderWhatsapp) => config.providers?.[id] || vazio;
  const atualizar = (id: ProviderWhatsapp, campo: keyof ProviderConfiguracao, valor: string) => setConfig(atual => ({ ...atual, providers: { ...atual.providers, [id]: { ...detalhe(id), [campo]: valor } } }));
