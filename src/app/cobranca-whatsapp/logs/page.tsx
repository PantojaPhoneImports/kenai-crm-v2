...
export default function LogsCobrancaPage() {
  ...
  useEffect(() => { if (!usuario || usuarioEhSocio(usuario)) { setLogs([]); return; } return onSnapshot(query(collection(db, "cobrancaLogs"), orderBy("data", "desc")), snap => setLogs(snap.docs.map(doc => ({ id: doc.id, ...doc.data() }) as Log))); }, [usuario]);
  ...
  if (loading) return <Layout><div className="h-64 animate-pulse rounded-2xl bg-zinc-900" /></Layout>;
